import { AfterContentInit, Component, EventEmitter, forwardRef,              } from '@angular/core'          ;
import { HostListener, Input, OnDestroy, Optional, Output, ViewEncapsulation } from '@angular/core'          ;
import { ControlValueAccessor, NG_VALUE_ACCESSOR                             } from '@angular/forms'         ;
import { PickerController, Form, Item, PickerColumn,                         } from 'ionic-angular'          ;
import { PickerCmp, PickerColumnCmp, IonicFormInput                          } from 'ionic-angular'          ;
...
export class MultiPicker implements AfterContentInit, ControlValueAccessor, OnDestroy, IonicFormInput {
  _disabled: any = false;
  _labelId: string = '';
  _text: string = '';
  _fn: Function;
  ...
  /**
   * @input {Array<string>} Header text to display above each column. Default: none.
   */
  @Input() headers: Array<string> = [];

  /**
   * @input {Array<string>} Width of the header strings. Example: '18px'. Default: ask DriftyCo what Ionic has as a default.
   */
  @Input() headerWidths: Array<string> = [];
  ...
  public generateSequence() {
    ...
    if (!this._isDependent || !hasParentCol) {
      ...
      for (let i = 0; i < this.multiPickerColumns.length; i++) {
        ...
        let headerWidth = this.headerWidths[i];
  ...
  public generate(picker: any) {
    ...
    this.multiPickerColumns.forEach((col, index) => {
      ...
      if(col.header !== undefined && col.header !== null && col.header !== '') {
        column.prefix = col.header;
      }
      if (col.headerWidth !== undefined && col.headerWidth !== null && col.headerWidth !== '') {
        column.prefixWidth = col.headerWidth;
      }
      // Set selectedIndex
   ...
   * @private
   */
  public getOptionParentValue(colIndex, option) {
   ...
   */
  public getParentCol(childColIndex: number, columns: PickerColumn[]): PickerColumn {
   ...
   * @private
   */
  public checkHasValue(inputValue: any) {
    if (this._item) {
  ...
  */
  public convertObjectToString(newData) {
    let value = ``;
    this.multiPickerColumns.forEach((col, index) => {
  ...
  public initFocus() {
    console.log("multi-picker: initFocus() called.");
  }
}
