...
//   //       if (this.seed && options.seed) options.seed += 1;
...
//   public pickSaturation (hue, options) {
...
//   public pickBrightness (H, S, options) {
...
//       case 'hsla':
//         let hslColor = this.HSVtoHSL(hsv);
...
//       case 'rgba':
//         let rgbColor = this.HSVtoRGB(hsv);
//         alpha = options.alpha || Math.random();
...
//   public getMinimumBrightness(H, S) {
...
//   public getHueRange(colorInput) {
//     if (typeof Number(colorInput) === 'number') {
//       let number = Number(colorInput);
...
//         let hue = this.HexToHSB(colorInput)[0];
//         return [ hue, hue ];
//       }
//     }
//     return [0,360];
...
//   public getSaturationRange(hue) {
//     return this.getColorInfo(hue).saturationRange;
//   }

//   public getColorInfo(hue) {
...
//   public defineColor(name, hueRange, lowerBounds) {
...
//   public HSVtoRGB(hsv:Array<number>) {
...
//   public HSVtoHSL(hsv:Array<number>) {
//     let h = hsv[0],
//       s = hsv[1]/100,
//       v = hsv[2]/100,
//       k = (2-s)*v;
...
//   public stringToInteger(input:string):number {
//     var total = 0;
...
export class ColorService {
  ...
  constructor() {
    ...
    window['RandomColor'] = this;
    // Populate the color dictionary
    this.loadColorBounds();
  }

  public randomColor(options:any) {
    options = options || {};
    // Check if there is a seed and ensure it's an
    // integer. Otherwise, reset the seed value.
    if (options.seed !== undefined && options.seed !== null && options.seed === Number(options.seed)) {
      this.seed = options.seed;
